...
namespace SIPSorcery.Media
{
    ...
    {
        public const int TEST_PATTERN_WIDTH = 704;
        public const int TEST_PATTERN_HEIGHT = 576;
        ...
        private const int STAMP_BOX_SIZE = 40;
        private const int STAMP_BOX_PADDING = 20;
...
#pragma warning restore CS0067
        ...
        public VideoSource(IVideoEncoder encoder = null)
        {
            if (encoder != null)
            {
            ...
            using var testPatternStm = File.Open($"3.264", FileMode.Open);
            using var memoryStream = new MemoryStream();
            testPatternStm.CopyTo(memoryStream);
            _testI420Buffer = memoryStream.ToArray();
        ...
        private void GenerateTestPattern(object state)
        {
            lock (_sendTestPatternTimer)
            {
                if (!_isClosed && (OnVideoSourceRawSample != null || OnVideoSourceEncodedSample != null))
                {
                    ...
                    var bytes = _testI420Buffer[
                        (_frameCount * TEST_PATTERN_WIDTH * TEST_PATTERN_HEIGHT * 3 / 2)..((_frameCount + 1) *
                            TEST_PATTERN_WIDTH * TEST_PATTERN_HEIGHT * 3 / 2)];

                    StampI420Buffer(bytes, TEST_PATTERN_WIDTH, TEST_PATTERN_HEIGHT, _frameCount);

                    if (OnVideoSourceRawSample != null)
                    {
                        GenerateRawSample(TEST_PATTERN_WIDTH, TEST_PATTERN_HEIGHT, bytes);
                    }
                        ...
                        !_formatManager.SelectedFormat.IsEmpty())
                    {
                        var encodedBuffer = _videoEncoder.EncodeVideo(TEST_PATTERN_WIDTH, TEST_PATTERN_HEIGHT,
                            bytes, VideoPixelFormatsEnum.I420, _formatManager.SelectedFormat.Codec);
